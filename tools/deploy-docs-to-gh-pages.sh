...
set +x
git clone "https://${{ secrets.GITHUB_TOKEN }}@github.com/esa/pyoptgra.git" pyoptgra_gh_pages -q
set -x
...
# to commit.
git commit -m "Update Sphinx documentation commit ${{ github.sha }} [skip ci]." || exit 0
PUSH_COUNTER=0
until git push -q
do
    git pull -q
